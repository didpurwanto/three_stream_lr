...
def sample_data(ori_arr, num_frames_per_clip, sample_rate):
 ...
 #   for i in range(int(num_frames_per_clip)):
...
def get_data(filename, num_frames_per_clip, sample_rate, s_index=-1):
    ...
    #            for i in range(num_frames_per_clip): # i =  0 : 63
    #                if i >= len(filenames):
    #                    i = len(filenames)-1
    ...
    filenames = sorted(filenames)
    #print(s_index)
    if s_index < 0 & len(filenames) > num_frames_per_clip:
        s_index = random.randint(num_frames_per_clip, (len(filenames) - num_frames_per_clip))
    else:
	s_index = 0
	#print(s_index,' ',len(filenames))
    for i in range(int(num_frames_per_clip/sample_rate)):
        #print(len(filenames))
...
def get_frames_data(filename, num_frames_per_clip, sample_rate, add_flow):
    ...
    filename_mi = os.path.join(filename, 'flowt')
    print(filename)
    rgb_ret_arr, s_index = get_data(filename_mi, num_frames_per_clip, sample_rate)
    if not add_flow:
        return rgb_ret_arr, [], s_index
    filename_x = os.path.join(filename, 'flowt')
    flow_x, _ = get_data(filename_x, num_frames_per_clip, sample_rate)
    flow_x = np.expand_dims(flow_x, axis=-1)
    
    filename_y = os.path.join(filename, 'flowt')
    flow_y, _ = get_data(filename_y, num_frames_per_clip, sample_rate)
    flow_y = np.expand_dims(flow_y, axis=-1)
    
    flow_x = flow_x[:,:,:,0,:] + flow_x[:,:,:,2,:]
    flow_y = flow_y[:,:,:,1,:] + flow_y[:,:,:,2,:]   
    flow_ret_arr = np.concatenate((flow_x, flow_y), axis=-1)
  #  print(flow_ret_arr.shape)
...
def read_clip_and_label(filename, batch_size, start_pos=-1, num_frames_per_clip=64, sample_rate=1, crop_size=224, shuffle=True, add_flow=True):
	...
	#print(line)
        ...
        tmp_label = line[1]
        if not shuffle:
            print("aaaLoading a video clip from {}...".format(dirname))

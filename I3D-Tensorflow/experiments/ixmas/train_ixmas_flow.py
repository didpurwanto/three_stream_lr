...
FLAGS = flags.FLAGS
model_save_dir = './models/HR_flow_imagenet_6000_6_64_0.0001_decay_split1'
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        learning_rate = tf.train.exponential_decay(FLAGS.learning_rate, global_step, decay_steps=2000, decay_rate=0.1, staircase=True)
    ...
    train_writer = tf.summary.FileWriter('./visual_logs/train_HR_flow_imagenet_6000_6_64_0.0001_decay_split1', sess.graph)
    test_writer = tf.summary.FileWriter('./visual_logs/test_HR_flow_imagenet_6000_6_64_0.0001_decay_split1', sess.graph)
    for step in xrange(FLAGS.max_steps):
        ...
        if (step+1) % 3000 == 0 or (step + 1) == FLAGS.max_steps:
            saver.save(sess, os.path.join(model_save_dir, 'i3d_ixmas_model'), global_step=step)
    print("done")
